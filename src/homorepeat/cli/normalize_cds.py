...
from homorepeat.io.tsv_io import ContractError, open_tsv_writer, read_tsv, write_tsv  # noqa: E402
...
def _run(args: argparse.Namespace) -> None:
    ...
    normalized_accessions: set[str] = set()

    with (
        open_tsv_writer(outdir / "sequences.tsv", fieldnames=SEQUENCES_FIELDNAMES) as sequence_writer,
        open_fasta_writer(normalized_cds_path) as cds_writer,
        ...
        for record in load_assembly_report(package_root):
            ...
            for header, sequence in iter_fasta(cds_path):
                ...
                sequence_writer.write_row(candidate_row)
                cds_writer.write_record(sequence_id, sequence)
                normalized_accessions.add(accession)
    ...
    _validate_normalized_accession_coverage(outdir / "download_manifest.tsv", normalized_accessions, args.batch_id)
